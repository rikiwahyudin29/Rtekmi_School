...
class HomeController extends Controller
{
    public function index()
    {
        ...
        $webProfil = Schema::hasTable('tbl_web_profil') 
                        ? DB::table('tbl_web_profil')->where('id', 1)->first() 
                        : null;
        
        $dataWeb = $webProfil ? (object) array_merge((array) $sekolah, (array) $webProfil) : $sekolah;

        // 2. DATA CMS
        $sliders = Schema::hasTable('tbl_slider') 
            ? DB::table('tbl_slider')->where('is_active', 1)->orderBy('urutan', 'ASC')->get() 
            : [];
        ...
        $galeri = Schema::hasTable('tbl_galeri') 
            ? DB::table('tbl_galeri')->orderBy('id', 'DESC')->limit(8)->get() 
            : [];

        // 3. STATISTIK REAL-TIME
        $stats = [
            'guru'      => DB::table('tbl_guru')->count(),
            'siswa'     => DB::table('tbl_siswa')->where('status_siswa', 'Aktif')->count(),
            'pendaftar' => Schema::hasTable('tbl_pendaftar') ? DB::table('tbl_pendaftar')->count() : 0,
