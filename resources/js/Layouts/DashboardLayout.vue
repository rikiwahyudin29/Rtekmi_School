    ...
    if (isSuperAdmin) {
        menus.push(
            ...
            { name: 'Manajemen Pengguna', icon: 'fas fa-users-cog', route: '#', active: route().current('admin.guru.*') || route().current('admin.siswa.*') || route().current('admin.roles.*') || route().current('admin.user-devices.*'), children: [
                ...
                { name: 'Manajemen Device', icon: 'fas fa-mobile-alt', route: 'admin.user-devices.index', active: route().current('admin.user-devices.index') },
                { name: 'Lokasi Pengguna', icon: 'fas fa-map-marked-alt', route: 'admin.user-devices.locations', active: route().current('admin.user-devices.locations') },
            ]}
        );
    }
