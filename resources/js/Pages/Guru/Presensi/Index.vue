...
const filterData = () => {
    form.get(route('guru.presensi.index'));
};
</script>
...
<template>
    <DashboardLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Riwayat Kehadiran (Guru)</h2>
        </template>
                                    ...
                                    <tr v-for="item in data" :key="item.id">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" 
                                                :class="{
                                                    'bg-green-100 text-green-800': item.status_kehadiran === 'Hadir',
                                                    'bg-red-100 text-red-800': item.status_kehadiran === 'Terlambat',
                                                    'bg-yellow-100 text-yellow-800': item.status_kehadiran === 'Izin' || item.status_kehadiran === 'Sakit',
