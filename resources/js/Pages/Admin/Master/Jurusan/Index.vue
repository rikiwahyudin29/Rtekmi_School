...
const openModal = (jrs = null) => {
    isEditing.value = !!jrs;
    if (jrs) {
        form.id = jrs.id;
        form.kode_jurusan = jrs.kode_jurusan;
        form.nama_jurusan = jrs.nama_jurusan;
        form.kepala_jurusan_id = jrs.kepala_jurusan_id || '';
        form.dapodik_id = jrs.dapodik_id || '';
    } else {
        form.reset();
        form.id = null;
        form.kepala_jurusan_id = '';
    }
...
const submit = () => {
    if (isEditing.value) {
        form.put(route('admin.master.jurusan.update', form.id), {
            onSuccess: () => closeModal(),
        });
    } else {
        form.post(route('admin.master.jurusan.store'), {
            onSuccess: () => closeModal(),
        });
    }
};
...
const deleteData = (id) => {
    if (confirm('Apakah Anda yakin ingin menghapus Jurusan ini?')) {
        router.delete(route('admin.master.jurusan.destroy', id));
    }
};
        ...
        <div class="py-8">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col h-[calc(100vh-10rem)]">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 shrink-0 gap-4">
                    <div>
                        <h2 class="font-bold text-2xl text-gray-900 dark:text-white tracking-tight">Data Jurusan</h2>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Kelola data program studi atau keahlian di sekolah.</p>
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col min-h-0 flex-1 overflow-hidden">
                    ...
                    <div class="p-5 border-b border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0 bg-gray-50/50 dark:bg-gray-800/50">
                        ...
                        <div class="relative w-full sm:w-72">
                            ...
                            <input v-model="search" type="text" class="w-full pl-10 pr-4 py-2 border-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 rounded-xl text-sm focus:ring-primary-500 focus:border-primary-500 transition-shadow" placeholder="Cari Jurusan...">
                        </div>
                    </div>
                        ...
                        <table class="w-full text-left text-sm whitespace-nowrap">
                            <thead class="sticky top-0 z-10">
                                <tr class="bg-gray-50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-300 text-xs uppercase tracking-wider border-b border-gray-100 dark:border-gray-700">
                                    ...
                                    <th class="py-4 px-5 font-bold">Nama Jurusan</th>
                                    <th class="py-4 px-5 font-bold">Kepala Jurusan</th>
                                    <th class="py-4 px-5 font-bold text-center w-32">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                                <tr v-for="(jrs, index) in jurusan.data" :key="jrs.id" class="hover:bg-gray-50/50 dark:hover:bg-gray-700/20 transition-colors group">
                                    <td class="py-4 px-5 text-gray-500 dark:text-gray-400 text-center">
                                        {{ (jurusan.current_page - 1) * jurusan.per_page + index + 1 }}
                                    </td>
                                    ...
                                    <td class="py-4 px-5 text-gray-500 dark:text-gray-400">
                                        {{ jrs.kepala_jurusan?.nama_lengkap || '-' }}
                                    </td>
                                    <td class="py-4 px-5 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <button @click="openModal(jrs)" class="w-8 h-8 rounded-lg bg-orange-50 text-orange-600 hover:bg-orange-100 flex items-center justify-center transition-colors dark:bg-orange-900/30 dark:text-orange-400 dark:hover:bg-orange-900/50">
                                                <i class="fas fa-edit text-xs"></i>
                                            </button>
                                            <button @click="deleteData(jrs.id)" class="w-8 h-8 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 flex items-center justify-center transition-colors dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-900/50">
                                                <i class="fas fa-trash text-xs"></i>
                                            </button>
                                        </div>
                                    </td>
                                </tr>
                                <tr v-if="jurusan.data.length === 0">
                    ...
                    <div class="p-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 flex flex-col sm:flex-row items-center justify-between gap-4 shrink-0">
                        <div class="text-sm text-gray-500 dark:text-gray-400">
                            Menampilkan <span class="font-bold text-gray-900 dark:text-white">{{ jurusan.from || 0 }}</span> sampai <span class="font-bold text-gray-900 dark:text-white">{{ jurusan.to || 0 }}</span> dari <span class="font-bold text-gray-900 dark:text-white">{{ jurusan.total }}</span> data
                        </div>
                        <div class="flex gap-1">
                            <Link v-for="(link, index) in jurusan.links" :key="index" :href="link.url || '#'" 
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
                ...
                <div class="relative inline-block w-full max-w-lg p-6 overflow-hidden text-left align-middle transition-all transform bg-white dark:bg-gray-800 shadow-xl rounded-2xl border border-gray-100 dark:border-gray-700">
                    <h3 class="text-lg font-bold leading-6 text-gray-900 dark:text-white mb-4">
                        {{ isEditing ? 'Edit' : 'Tambah' }} Jurusan
                    </h3>
                    
                    <form @submit.prevent="submit" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Kode Jurusan <span class="text-red-500">*</span></label>
                                <input v-model="form.kode_jurusan" type="text" class="mt-1 block w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Contoh: RPL" required>
                                <div v-if="form.errors.kode_jurusan" class="text-red-500 text-xs mt-1">{{ form.errors.kode_jurusan }}</div>
                            </div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Jurusan <span class="text-red-500">*</span></label>
                            <input v-model="form.nama_jurusan" type="text" class="mt-1 block w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Contoh: Rekayasa Perangkat Lunak" required>
                            <div v-if="form.errors.nama_jurusan" class="text-red-500 text-xs mt-1">{{ form.errors.nama_jurusan }}</div>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Kepala Jurusan</label>
                            <select v-model="form.kepala_jurusan_id" class="mt-1 block w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                                <option value="">-- Pilih Kepala Jurusan --</option>
                                <option v-for="guru in gurus" :key="guru.id" :value="guru.id">
                                    {{ guru.nama_lengkap }} ({{ guru.nip || '-' }})
                                </option>
                            </select>
                            <div v-if="form.errors.kepala_jurusan_id" class="text-red-500 text-xs mt-1">{{ form.errors.kepala_jurusan_id }}</div>
                        </div>
