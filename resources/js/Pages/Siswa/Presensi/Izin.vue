...
const submit = () => {
    form.post(route('siswa.presensi.ajukan'), {
        onSuccess: () => {
            form.reset();
        }
    });
...
<template>
    <DashboardLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Pengajuan Izin / Sakit (Siswa)</h2>
        </template>
                        ...
                        <div class="p-6">
                            ...
                            <form @submit.prevent="submit" class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <textarea v-model="form.keterangan" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required placeholder="Tulis alasan izin/sakit..."></textarea>
                                </div>
                                <div>
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="item in riwayat" :key="item.id">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="item.status_kehadiran == 'Izin' ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800'">
